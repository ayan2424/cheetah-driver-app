...
<p>The <strong>Cheetah Driver & Warehouse Picker Mobile App</strong> is an enterprise-grade, high-performance cross-platform application developed with <strong>Flutter 3.x</strong> and <strong>Dart 3.x</strong>. Designed specifically for courier delivery fleets, logistics companies, express parcel services, and 3PL fulfillment centers, it provides a seamless dual-role interface for both <strong>Delivery Drivers / Riders</strong> and <strong>WMS Warehouse Pickers</strong>.</p>

<p>Whether connected natively to the <strong>Cheetah Courier Management System</strong> backend or integrated into your own custom REST API, this app delivers 60/120 FPS fluid animations, hardware-level GPS anti-tampering enforcement, digital customer signatures, drop-off photo proof of delivery (POD), live camera barcode scanning, and zero-data-loss AES-256 encrypted offline queueing.</p>
...
<ul>
  ...
  <li><strong>Proof of Delivery (POD) Suite:</strong>
    <ul>
      ...
      <li>Drop-off photo capture via device camera or gallery upload.</li>
  ...
  <li><strong>Cash on Delivery (COD) Wallet Ledger:</strong> Real-time tracking of collected COD cash, pending branch vault deposits, and driver payout transaction history.</li>
  <li><strong>Live Background GPS Telemetry:</strong> Transmits real-time coordinates, speed, and heading to the central dispatcher fleet map.</li>
</ul>
...
<ul>
  ...
  <li><strong>Precise Spatial Navigation:</strong> Visual warehouse shelf hierarchy (<em>Zone &rarr; Aisle &rarr; Rack &rarr; Shelf &rarr; Bin</em>) to minimize picker walking time.</li>
  ...
  <li><strong>Fulfillment Pipeline:</strong> 1-tap status toggling (<em>In Progress</em>, <em>Completed</em>, <em>Out of Stock Issue</em>).</li>
</ul>
...
<ul>
  ...
  <li>Android Project Workspace (Gradle, ProGuard rules, Android 14/15 target).</li>
...
• Real-time Cash on Delivery (COD) collection wallet & settlement ledger
...
• 10+ Multi-language internationalization including full RTL support (Arabic, Urdu)
